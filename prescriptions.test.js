...
describe("applyDiscount", () => {
  it("applies a 25% discount if the customer has subscription", () => {
    expect(applyDiscount(100, true)).toBe(75); 
    expect(applyDiscount(10, true)).toBe(7.5); 
  });

  it("does not applydiscount if the customer does not have a subscription", () => {
...
describe("applyCoupon", () => {
  it("applies a $10 discount if the customer has a coupon", () => {
    expect(applyCoupon(100, true)).toBe(90);
    expect(applyCoupon(10, true)).toBe(0); 
  });

  it("does not apply discount if the customer does not have a coupon", () => {
    expect(applyCoupon(100, false)).toBe(100); 
    expect(applyCoupon(10, false)).toBe(10); 
  });
